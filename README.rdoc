...
Currently only the ´mongo_mapper´ option has been tested. Note if you run ´$ rake test´, you will still get a few errors and failures.
This is not due to any errors in ´mm-devise´, but rather to a few "issues" with the devise tests themselves IMO. 
...
either ´mongo_mapper´ or ´mongo_mapper_active_model´ (ActiveModel::Validations) 
